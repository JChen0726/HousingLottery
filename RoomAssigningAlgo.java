...
public class RoomAssigningAlgo {
    ...
    RoomAssigningAlgo(ArrayList<Object> Student, ArrayList<Room> Dorms) { //CHANGE TO ARRAYLIST OF OBJECTS
        // for single students
        choicesWeight = new int[]{1, 3, 5, 7};
        gradeWeight = new int[]{1, 3, 5};
        this.dormRooms = Dorms;
        this.costMatrix = new int[Student.size()][Dorms.size()];
    ...
    private int[][] makeSquare(int[][] inputmatrix) {
        int colCount = inputmatrix[1].length;
        ...
        for (int i = 0; i < squareCM.length; i++) {
            for (int j = 0; j < squareCM[0].length; j++) {
                try {
                    squareCM[i][j] = this.costMatrix[i][j];
    ...
    public void runCalculation() {
        ...
        int[][] stayassignments = hungarianAlgorithm.findOptimalAssignment();
        // this is essentially stay lottery.
        for (int i = 0; i < stayassignments.length; i++) {
            int roomIdx = stayassignments[i][1];
            this.dormRooms.get(roomIdx).assignStudents(students.get(i));
        }
    }
